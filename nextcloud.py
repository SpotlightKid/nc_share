...
#
"""ownCloud/Nextcloud client module

Makes it possible to access files on a remote ownCloud/Nextcloud instance,
...
class Client(object):
    ...
    def share_file_with_link(self, path, **kwargs):
        ...
        if res.status_code == 200:
            resp_text = res.content.decode('utf-8')

            if self._debug:
                print("OCS response: {}".format(self.OCS_SERVICE_SHARE, resp_text))

            tree = ET.fromstring(resp_text)
            self._check_ocs_status(tree)
            return self._get_shareinfo(tree.find('data'))
        raise HTTPResponseError(res)
